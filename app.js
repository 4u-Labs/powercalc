...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (chrome && chrome.storage && chrome.storage.local) {
        chrome.storage.local.get(['activeMode', 'theme', 'appLanguage', 'exchangeRates', 'lastRatesSync', 'calcHistory', 'numberFormat', 'decimalPlaces', 'scientificMemory'], (result) => {
            if (result.appLanguage) {
                setLanguage(result.appLanguage);
            } else {
                const userNavLang = navigator.language || navigator.userLanguage || 'pt-BR';
                setLanguage(userNavLang.startsWith('en') ? 'en-US' : 'pt-BR');
            }
    ...
    const i18n = {
        'pt-BR': {
            ...
            'nav.percentage': 'Porcent.',
            'nav.finance': 'Finanças',
            'nav.dates': 'Datas',
            'nav.health': 'Saúde',
            'nav.matrices': 'Matrizes',
            'nav.programmer': 'Program.',
            'nav.engineering': 'Engenh.',
            'tab.config': '⚙️ Ajustes',
            'tab.guide': '📖 Guia Técnico',
            'lbl.about_title': '⚡ Sobre o PowerCalc',
            'lbl.about_desc': 'PowerCalc é uma calculadora premium multifuncional tudo-em-um desenvolvida para simplificar e agilizar suas tarefas diárias de cálculo e conversão.',
            'lbl.language': 'Idioma / Language',
            'lbl.num_format': 'Formato Numérico',
            'lbl.decimals': 'Casas Decimais',
            'lbl.auto_decimals': 'Automático (Até 8)',
            'lbl.close': 'Fechar',
            'lbl.btn_calculate': 'CALCULAR'
        },
        'en-US': {
            'nav.simple': 'Basic',
            ...
            'nav.percentage': 'Percent',
            'nav.finance': 'Finance',
            'nav.dates': 'Dates',
            'nav.health': 'Health',
            'nav.matrices': 'Matrices',
            'nav.programmer': 'Programmer',
            'nav.engineering': 'Engineers',
            'tab.config': '⚙️ Settings',
            'tab.guide': '📖 User Guide',
            'lbl.about_title': '⚡ About PowerCalc',
            'lbl.about_desc': 'PowerCalc is an all-in-one premium multi-function calculator designed for everyday math, advanced engineering, and financial simulations.',
            'lbl.language': 'Language / Idioma',
            'lbl.num_format': 'Number Format',
            'lbl.decimals': 'Decimal Places',
            'lbl.auto_decimals': 'Automatic (Up to 8)',
            'lbl.close': 'Close',
            'lbl.btn_calculate': 'CALCULATE'
        }
    };

    function setLanguage(lang) {
        currentLang = (lang && lang.startsWith('en')) ? 'en-US' : 'pt-BR';
        ...
        // Update nav labels
        const navMap = {
            'simple': dict['nav.simple'],
            'scientific': dict['nav.scientific'],
            'units': dict['nav.units'],
            'currency': dict['nav.currency'],
            'statistics': dict['nav.statistics'],
            'percentage': dict['nav.percentage'],
            'finance': dict['nav.finance'],
            'dates': dict['nav.dates'],
            'health': dict['nav.health'],
            'matrices': dict['nav.matrices'],
            'programmer': dict['nav.programmer'],
            'engineering': dict['nav.engineering']
        };

        document.querySelectorAll('.nav-item').forEach(btn => {
            const mode = btn.getAttribute('data-mode');
            const labelSpan = btn.querySelector('.nav-label');
            if (labelSpan && navMap[mode]) {
                labelSpan.innerText = navMap[mode];
            }
        });

        // Update modal tabs and labels
        const btnTabModalConfig = document.getElementById('btnTabModalConfig');
        const btnTabModalGuide = document.getElementById('btnTabModalGuide');
        const btnCloseConfig = document.getElementById('btnCloseConfig');
        if (btnTabModalConfig) btnTabModalConfig.innerText = dict['tab.config'];
        if (btnTabModalGuide) btnTabModalGuide.innerText = dict['tab.guide'];
        if (btnCloseConfig) btnCloseConfig.innerText = dict['lbl.close'];
